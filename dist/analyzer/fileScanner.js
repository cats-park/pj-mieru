...
export class FileScanner {
    supportedExtensions = ['.vue', '.jsx', '.tsx', '.js', '.ts'];
    ...
    async scanProject(projectPath, options = {}) {
        ...
        try {
            ...
            // ファイルスキャンを実行
            await this.scanDirectory(projectPath, projectPath, files, errors, options);
            ...
            errors.push(`スキャンエラー: ${error instanceof Error ? error.message : String(error)}`);
            return {
                files,
                errors,
                totalFiles: 0,
     ...
     * @param options スキャンオプション
     */
    async scanDirectory(currentPath, rootPath, files, errors, options) {
        try {
                ...
                if (entry.isDirectory()) {
                    ...
                    if (this.isSupportedFile(entry.name, options)) {
                        try {
                            ...
                            errors.push(`ファイル情報取得エラー (${relativePath}): ${error instanceof Error ? error.message : String(error)}`);
                        }
                    }
                }
            }
            ...
            errors.push(`ディレクトリ読み取りエラー (${path.relative(rootPath, currentPath)}): ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * プロジェクトパスの妥当性を検証
     ...
     * @returns 無視する場合はtrue
     */
    shouldIgnore(name, relativePath, options) {
     ...
     */
    isSupportedFile(fileName, options) {
