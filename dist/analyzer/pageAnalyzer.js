import { ReactRouterAnalyzer } from './reactRouterAnalyzer.js';
...
export class PageAnalyzer {
    ...
    detectProjectType(files) {
        ...
        // ディレクトリ構造からも判定
        const hasNextjsStructure = files.some(f => f.path.includes('/pages/') ||
            f.path.includes('/app/') ||
            f.path.includes('next.config'));
        const hasNuxtStructure = files.some(f => f.path.includes('nuxt.config') ||
            f.path.includes('.nuxt/'));
        if (hasNextjsStructure)
            return 'next';
        if (hasNuxtStructure)
            return 'nuxt';
        if (hasVue)
     ...
     * React系プロジェクトの解析
     */
    async analyzeReactPages(files, astResults, startTime) {
     ...
     */
    buildPageStructureFromRouting(routerResult, files, astResults, startTime) {
        const pages = new Map();
        // 各ルートからPageInfoを構築
     ...
     */
    buildComponentsFromDependencies(dependencies, files) {
        const components = [];
     ...
     * Vue系プロジェクトの解析（従来の方法）
     */
    async analyzeVuePages(files, astResults, vueResults, startTime) {
        // Identify page files
    ...
    isComponentImport(source) {
        ...
        if (lowerSource.includes('component') ||
