...
Return ONLY valid JSON in this exact format:
{
  ...
  "componentPatterns": ["Array of component file patterns"]
    ...
    async analyzePageComponents(pages, files, framework) {
        ...
        for (const page of pages) {
            try {
                ...
                const components = await this.extractComponentsRecursively(content, page.name, framework, files, visitedComponents, 0, // 深度
                5, // 最大深度
                [] // 現在のパス
                );
    ...
    async extractComponentsRecursively(content, componentName, framework, files, visitedComponents, depth, maxDepth = 5, currentPath = []) {
        // 深度制限チェック
        if (depth > maxDepth) {
...
CRITICAL: Response must be ONLY the JSON object, nothing else.`;
            ...
            for (const comp of result.components) {
                ...
                if (componentFile) {
                    try {
                        ...
                        // 再帰的にコンポーネントを解析
                        component.children = await this.extractComponentsRecursively(componentContent, comp.name, framework, files, visitedComponents, depth + 1, maxDepth, newPath);
                    }
                    catch (error) {
    ...
    findComponentFile(estimatedPath, componentName, files) {
        // 1. 推定パスで直接探す
        if (estimatedPath) {
            const normalizedPath = estimatedPath.replace(/^\.\//, '').replace(/^\//, '');
            const file = files.find(f => f.relativePath === estimatedPath ||
    ...
    isComposableFunction(componentName) {
    ...
    isExternalLibraryComponent(componentName, filePath) {
        // 外部ライブラリの典型的なパターン
    ...
    extractComponentsWithRegex(content) {
        const components = [];
