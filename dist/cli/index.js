...
// Markdownレポート生成関数
function generateMarkdownReport(result, projectName, inputPath) {
...
- **総ファイル数**: ${totalFiles}
- **ページ数**: ${pages.length}
- **総コンポーネント数**: ${pages.reduce((sum, page) => sum + page.components.length, 0)}
...
// Mermaid図生成関数
function generateMermaidDiagram(pages) {
    if (pages.length === 0) {
        return '';
    }
    let mermaid = `
...
function renderComponentHierarchy(component, depth) {
