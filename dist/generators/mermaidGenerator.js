...
export class MermaidGenerator {
    ...
    generateDiagram(dependencyGraph, options = {}) {
    ...
    createMermaidNodes(dependencyGraph) {
        const circularFiles = this.getCircularDependencyFiles(dependencyGraph.circularDependencies);
        return Array.from(dependencyGraph.nodes.values()).map((node) => {
            const fileExt = path
    ...
    createMermaidEdges(dependencyGraph) {
        ...
        dependencyGraph.edges.forEach((edge) => {
            ...
            if (!edgeMap.has(edgeKey)) {
                edgeMap.set(edgeKey, {
                    ...
                    to: toId,
                    type: edge.type,
                    style: edge.type === 'component-usage'
    ...
    generateMermaidSyntax(diagram, options) {
        const lines = [];
        ...
        if (options.groupByDirectory) {
            ...
            Object.entries(groups).forEach(([dir, nodes]) => {
                lines.push(`  subgraph ${dir.replace(/[^a-zA-Z0-9]/g, '_')}_group ["${dir}"]`);
                nodes.forEach((node) => {
    ...
    getCircularDependencyFiles(circularDeps) {
        const files = new Set();
