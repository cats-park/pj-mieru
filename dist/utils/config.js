...
export class ConfigManager {
    static configPath = path.join(os.homedir(), '.mierurc.json');
    static async getApiKey(cliApiKey) {
        // 1. CLI引数が最優先
        if (cliApiKey) {
            return cliApiKey;
        }
...
OpenAI APIキーの取得方法: https://platform.openai.com/api-keys
    ...
    static async saveConfig(config) {
        try {
            await fs.promises.writeFile(this.configPath, JSON.stringify(config, null, 2), { mode: 0o600 } // ファイルパーミッションを制限
            );
