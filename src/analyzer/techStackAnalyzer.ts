...
export class TechStackAnalyzer {
  async analyzeTechStack(projectPath: string, files: ScannedFile[]): Promise<TechStackInfo> {
  ...
  private analyzeLanguages(files: ScannedFile[]): LanguageInfo[] {
  ...
  private async analyzeFrameworksWithLLM(
    projectPath: string,
    packageJson: any,
    files: ScannedFile[]
  ...
  private buildFrameworkAnalysisPrompt(context: any): string {
...
Return only the JSON array, no additional text.`;
  ...
  private parseLLMFrameworkResponse(response: string): FrameworkInfo[] {
    try {
      ...
      const frameworks = JSON.parse(jsonMatch[0]) as FrameworkInfo[];
  ...
  private async readConfigFiles(projectPath: string, files: ScannedFile[]): Promise<Record<string, any>> {
  ...
  private analyzeFileStructure(files: ScannedFile[]): any {
    ...
    files.forEach(file => {
      ...
      // Track special files
      if (['pages', 'app', 'src', 'components', 'layouts', 'plugins', 'middleware'].some(dir => 
  ...
  private async extractCodeSnippets(projectPath: string, files: ScannedFile[]): Promise<Record<string, string>> {
  ...
  private getDirectoryStructure(files: ScannedFile[]): string[] {
  ...
  private async analyzeBuildTools(projectPath: string, files: ScannedFile[]): Promise<BuildToolInfo[]> {
  ...
  private determinePrimaryFramework(frameworks: FrameworkInfo[]): string {
    if (frameworks.length === 0) return 'Unknown';
    
    // メタフレームワークを優先（Next.js, Nuxt.js, Gatsby など）
    const metaFrameworks = frameworks.filter(f => 
      ['Next.js', 'Nuxt.js', 'Gatsby', 'SvelteKit'].includes(f.name)
    );
