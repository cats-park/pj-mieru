...
Return ONLY valid JSON in this exact format:
{
  ...
  private async identifyPages(files: ScannedFile[], framework: FrameworkInfo): Promise<PageInfo[]> {
    ...
    const candidateFiles = files.filter(file => 
      /\.(js|jsx|ts|tsx|vue)$/.test(file.path) &&
...
Return ONLY valid JSON in this exact format:
{
  ...
  private async extractComponentsFromPage(content: string, pageName: string, framework: FrameworkInfo): Promise<ComponentInfo[]> {
...
Return ONLY valid JSON in this exact format:
{
  "components": [
    ...
    try {
      ...
      return result.components.map((comp: any) => ({
        name: comp.name,
        type: comp.type,
