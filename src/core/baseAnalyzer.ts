...
export class BaseAnalyzer {
  async analyzeProject(
    ...
    if (verbose) {
      console.log(
        `📊 ${scanResult.totalFiles} ファイルを発見 (${scanResult.scanDuration}ms)`
      );
    }
    ...
    const astBatchResult = await astParser.analyzeFiles(
    ...
    if (verbose) {
      console.log(
        `📊 AST解析: ${astBatchResult.successCount} 成功, ${astBatchResult.errorCount} エラー`
      );
      console.log(
        `📊 Vue解析: ${vueResults.filter((r) => r.success).length} 成功, ${vueResults.filter((r) => !r.success).length} エラー`
      );
    }
